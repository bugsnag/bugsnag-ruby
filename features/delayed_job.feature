...
Scenario: An unhandled RuntimeError sends a report
  ...
  And the event "metaData.job.payload.display_name" equals "TestModel.fail"
  And the event "metaData.job.payload.method_name" equals "fail"
  And the payload field "events.0.metaData.job.payload.args" is an array with 0 elements

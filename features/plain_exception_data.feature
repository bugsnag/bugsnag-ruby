...
Scenario Outline: An error has built in meta-data
  ...
  And I run the command "plain-ruby bundle exec ruby exception_data/<state>_meta_data.rb" on the service "features/fixtures/plain/docker-compose.yml"
...
Scenario Outline: An error has built in context
  And I set environment variable "RUBY_VERSION" to "<ruby version>"
  And I build the service "plain-ruby" from the compose file "features/fixtures/plain/docker-compose.yml"
  And I run the command "plain-ruby bundle exec ruby exception_data/<state>_context.rb" on the service "features/fixtures/plain/docker-compose.yml"
  And I wait for 1 second
  Then I should receive a request
  And the request used the Ruby notifier
...
Scenario Outline: An error has built in grouping hash
  And I set environment variable "RUBY_VERSION" to "<ruby version>"
  And I build the service "plain-ruby" from the compose file "features/fixtures/plain/docker-compose.yml"
  And I run the command "plain-ruby bundle exec ruby exception_data/<state>_hash.rb" on the service "features/fixtures/plain/docker-compose.yml"
  And I wait for 1 second
  Then I should receive a request
  And the request used the Ruby notifier
  And the request contained the api key "a35a2a72bd230ac0aa0f52715bbdc6aa"
...
Scenario Outline: An error has built in user id
  And I set environment variable "RUBY_VERSION" to "<ruby version>"
  And I build the service "plain-ruby" from the compose file "features/fixtures/plain/docker-compose.yml"
  And I run the command "plain-ruby bundle exec ruby exception_data/<state>_user_id.rb" on the service "features/fixtures/plain/docker-compose.yml"
  And I wait for 1 second
  Then I should receive a request
  And the request used the Ruby notifier
  And the request used payload v4 headers

...
Scenario Outline: Metadata can be added to a report using add_tab
  ...
  And I run the command "plain-ruby bundle exec ruby report_modification/add_tab.rb" on the service "features/fixtures/plain/docker-compose.yml"
...
Scenario Outline: Metadata can be added to an existing tab using add_tab
  ...
  And I set environment variable "CALLBACK_INITIATOR" to "<initiator>"
  And I build the service "plain-ruby" from the compose file "features/fixtures/plain/docker-compose.yml"
  And I run the command "plain-ruby bundle exec ruby report_modification/add_tab_existing.rb" on the service "features/fixtures/plain/docker-compose.yml"
  And I wait for 1 second
  Then I should receive a request
  And the request used the Ruby notifier
  And the request used payload v4 headers
...
Scenario Outline: Metadata can be overwritten using add_tab
  ...
  And I set environment variable "CALLBACK_INITIATOR" to "<initiator>"
  And I build the service "plain-ruby" from the compose file "features/fixtures/plain/docker-compose.yml"
  And I run the command "plain-ruby bundle exec ruby report_modification/add_tab_override.rb" on the service "features/fixtures/plain/docker-compose.yml"
  And I wait for 1 second
  Then I should receive a request
  And the request used the Ruby notifier
  And the request used payload v4 headers
